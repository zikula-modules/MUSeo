 ...
 *
 * This is the base repository class for module entities.
 */
class MUSeo_Entity_Repository_Base_Module extends EntityRepository
{
    /**
    ...
    public function truncateTable()
    {
        $query = $this->getEntityManager()
                 ->createQuery('DELETE MUSeo_Entity_Module');
        $query->execute();
    }
     ...
     *
     * @return array|MUSeo_Entity_Module retrieved data array or MUSeo_Entity_Module instance
     */
     ...
     * @param boolean $useJoins Whether to include joining related objects (optional) (default=true).
     *
     * @return ArrayCollection collection containing retrieved MUSeo_Entity_Module instances
     */
    ...
    public function selectSearch($fragment = '', $exclude = array(), $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.controllerForView LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.controllerForSingleObject LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.parameterForObjects LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.nameOfIdentifier LIKE \'%' . $fragment . '%\'';
        $whereSub .= ((!empty($whereSub)) ? ' OR ' : '') . 'tbl.extraIdentifier LIKE \'%' . $fragment . '%\'';
    ...
    public function selectCount($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numModules';
        if ($useJoins === true) {
     ...
     * @param int    $excludeid  Id of modules to exclude (optional).
