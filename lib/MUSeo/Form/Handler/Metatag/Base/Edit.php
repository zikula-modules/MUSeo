 ...
 */
class MUSeo_Form_Handler_Metatag_Base_Edit extends MUSeo_Form_Handler_Common_Edit
{
    /**
     * Pre-initialise hook.
     *
    ...
    public function preInitialize()
    {
        ...
        $this->objectType = 'metatag';
        $this->objectTypeCapital = 'Metatag';
        $this->objectTypeLower = 'metatag';
    
        $this->hasPageLockSupport = true;
        // array with upload fields and mandatory flags
        $this->uploadFields = array('facebookImage' => false, 'twitterImage' => false, 'googlePlusImage' => false);
        // array with list fields and multiple flags
        $this->listFields = array('workflowState' => false, 'robotsIndex' => false, 'robotsFollow' => false, 'robotsAdvanced' => true);
    }
    ...
    public function initialize(Zikula_Form_View $view)
    {
        ...
        if ($this->mode == 'create') {
            $modelHelper = new MUSeo_Util_Model($this->view->getServiceManager());
            if (!$modelHelper->canBeCreated($this->objectType)) {
                LogUtil::registerError($this->__('Sorry, but you can not create the metatag yet as other items are required which must be created before!'));
        ...
        if (count($this->listFields) > 0) {
            $helper = new MUSeo_Util_ListEntries($this->view->getServiceManager());
    ...
    protected function getDefaultMessage($args, $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                        if ($this->mode == 'create') {
                            $message = $this->__('Done! Metatag created.');
                        } else {
                            $message = $this->__('Done! Metatag updated.');
                        }
                        break;
            case 'delete':
                        $message = $this->__('Done! Metatag deleted.');
                        break;
            default:
                        $message = $this->__('Done! Metatag updated.');
                        break;
    ...
    public function applyAction(array $args = array())
    {
        ...
        try {
            ...
            $workflowHelper = new MUSeo_Util_Workflow($this->view->getServiceManager());
