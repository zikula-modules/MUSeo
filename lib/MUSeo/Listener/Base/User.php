 ...
 */
class MUSeo_Listener_Base_User
{
    /**
    ...
    public static function delete(Zikula_Event $event)
    {
        ...
        $repo = $entityManager->getRepository('MUSeo_Entity_Metatag');
        // set creator to admin (2) for all metatags created by this user
        $repo->updateCreator($uid, 2);
        
        // set last editor to admin (2) for all metatags updated by this user
        $repo->updateLastEditor($uid, 2);
        
        $repo = $entityManager->getRepository('MUSeo_Entity_Extension');
        // set creator to admin (2) for all extensions created by this user
        $repo->updateCreator($uid, 2);
        
        // set last editor to admin (2) for all extensions updated by this user
        $repo->updateLastEditor($uid, 2);
    }
}
