...
class MUSeo_Api_Scoring extends Zikula_AbstractApi
{
	...
	private function score_keyword($keyword) {
		...
		if ($containingStopword !== false) {
			$this->save_score_result(5, __f('The keyword for this page contains one or more %sstop words%s, consider removing them. Found \'%s\'.', array('<a href="http://en.wikipedia.org/wiki/Stop_words">', '</a>', $containingStopword)), 'keyword_stopwords');
		}
	}
	...
	private function score_title($title) {
		...
		if ($title == '') {
			$this->save_score_result( 1, __('Please create a page title.'), 'title');
		} else {
	
			$length = $this->statistics->text_length($title);
			if ($length < $scoreTitleMinLength) {
				$this->save_score_result(6, __f('The page title contains %d characters, which is less than the recommended minimum of 40 characters. Use the space to add keyword variations or create compelling call-to-action copy.', array($length)), 'title_length');
			} elseif ($length > $scoreTitleMaxLength) {
				$this->save_score_result(6, __f('The page title contains %d characters, which is more than the viewable limit of 70 characters; some words will not be visible to users in your listing.', array($length)), 'title_length');
			} else {
				$this->save_score_result(9, __('The page title is more than 40 characters and less than the recommended 70 character limit.'), 'title_length');
			}
			...
			if ($needle_position === false) {
				$this->save_score_result(2, __f('The keyword / phrase %s does not appear in the page title.', array($this->keywordFolded)), 'title_keyword');
			} elseif ($needle_position <= $scoreTitleKeywordLimit) {
				$this->save_score_result(9, __('The page title contains keyword / phrase, at the beginning which is considered to improve rankings.'), 'title_keyword');
			} else {
				$this->save_score_result(6, __('The page title contains keyword / phrase, but it does not appear at the beginning; try and move it to the beginning.'), 'title_keyword');
			}
		}
	}
	...
	private function score_description($description, $maxlength = 155) {
		...
		if ($description == '') {
			$this->save_score_result(1, __('No meta description has been specified, search engines will display copy from the page instead.'), 'description_length');
		} else {
			$length = $this->statistics->text_length($description);
	
			if ($length < $scoreDescriptionMinLength) {
				$this->save_score_result(6, __f('The meta description is under 120 characters, however up to %s characters are available. %s', array($maxlength, $metaShorter)), 'description_length');
			} elseif ($length <= $maxlength) {
				$this->save_score_result(9, __('In the specified meta description, consider: How does it compare to the competition? Could it be made more appealing?'), 'description_length');
			} else {
				$this->save_score_result(6, __f('The specified meta description is over %s characters, reducing it will ensure the entire description is visible. %s', array($maxlength, $metaShorter)), 'description_length');
			}
			...
			if (strrpos($haystack1, $this->keywordFolded) === false && strrpos($haystack2, $this->keywordFolded) === false) {
				$this->save_score_result(3, __('A meta description has been specified, but it does not contain the target keyword / phrase.'), 'description_keyword');
			} else {
				$this->save_score_result(9, __('The meta description contains the primary keyword / phrase.'), 'description_keyword');
			}
		}
	}
	...
	private function score_body($body, $firstp) {
		...
		if ($wordCount < $lengthScore['bad']) {
			$this->save_score_result(- 20, __f('There are %d words contained in the body copy. This is far too low and should be increased.', array($wordCount, $lengthScore['good'])), 'body_length', array($wordCount));
		} elseif ($wordCount < $lengthScore['poor']) {
			$this->save_score_result(- 10, __f('There are %d words contained in the body copy, this is below the %d word recommended minimum. Add more useful content on this topic for readers.', array($wordCount, $lengthScore['good'])), 'body_length', $wordCount);
		} elseif ($wordCount < $lengthScore['ok']) {
			$this->save_score_result(5, __f('There are %d words contained in the body copy, this is below the %d word recommended minimum. Add more useful content on this topic for readers.', array($wordCount, $lengthScore['good'])), 'body_length', $wordCount);
		} elseif ($wordCount < $lengthScore['good']) {
			$this->save_score_result(7, __f('There are %d words contained in the body copy, this is slightly below the %d word recommended minimum, add a bit more copy.', array($wordCount, $lengthScore['good'])), 'body_length', $wordCount);
		} else {
			$this->save_score_result(9, __f('There are %d words contained in the body copy, this is more than the %d word recommended minimum.', array($wordCount, $lengthScore['good'])), 'body_length', $wordCount);
		}
		...
		if ($keywordWordCount > 10) {
			$this->save_score_result(0, __('Your keyphrase is over 10 words, a keyphrase should be shorter and there can be only one keyphrase.'), 'focus_keyword_length');
		} else {
			...
			if ($wordCount > 100) {
				...
				if ($keywordDensity < 1) {
					$this->save_score_result(4, __f('The keyword density is %s%%, which is a bit low, the keyword was found %s times.', array($keywordDensity, $keywordCount)), 'keyword_density');
				} elseif ($keywordDensity > 4.5) {
					$this->save_score_result(- 50, __f('The keyword density is %s%%, which is over the advised 4.5%% maximum, the keyword was found %s times.', array($keywordDensity, $keywordCount)), 'keyword_density');
				} else {
					$this->save_score_result(9, __f('The keyword density is %s%%, which is great, the keyword was found %s times.', array($keywordDensity, $keywordCount)), 'keyword_density');
				}
			}
		}
		...
		if (preg_match('`\b' . preg_quote($this->keyword, '`') . '\b`miu', $firstp) || preg_match('`\b' . preg_quote($this->keyword, '`') . '\b`mi', $firstp) || preg_match('`\b' . preg_quote($this->keywordFolded, '`') . '\b`miu', $firstp)
			...
			$this->save_score_result(9, __('The keyword appears in the first paragraph of the copy.'), 'keyword_first_paragraph');
		} else {
			$this->save_score_result(3, __('The keyword doesn\'t appear in the first paragraph of the copy, make sure the topic is clear immediately.'), 'keyword_first_paragraph');
		}
		...
		if (substr(ZLanguage::getLanguageCode(), 0, 2) == 'en' && $wordCount > 100) {
			...
			$this->save_score_result($score, __f('The copy scores %s in the %s test, which is considered %s to read. %s', array($flesch, '<a href="http://en.wikipedia.org/wiki/Flesch-Kincaid_readability_test#Flesch_Reading_Ease">' . __('Flesch Reading Ease') . '</a>', $level, $note)), 'flesch_kincaid');
		}
	}
	...
	private function score_url() {	
		...
		if (stripos($haystack1, $needle) || stripos($haystack2, $needle)) {
			$this->save_score_result(9, __('The keyword / phrase appears in the URL for this page.'), 'url_keyword');
		} else {
			$this->save_score_result(6, __('The keyword / phrase does not appear in the URL for this page. If you decide to rename the URL be sure to check the old URL 301 redirects to the new one!'), 'url_keyword');
		}
	
		/*
		// Check for Stop Words in the slug
		if ($wpseo_admin->stopwords_check($job['pageSlug'], true) !== false) {
			$this->save_score_result(5, __('The slug for this page contains one or more <a href="http://en.wikipedia.org/wiki/Stop_words">stop words</a>, consider removing them.'), 'url_stopword');
		}
		...
		if (($this->statistics->text_length($this->keyword) + 20) < $this->statistics->text_length($job['pageSlug']) && 40 < $this->statistics->text_length($job['pageSlug'])) {
			$this->save_score_result(5, __('The slug for this page is a bit long, consider shortening it.'), 'url_length');
		}*/
	}
	...
	private function score_anchor_texts($anchor_texts, $count) {
		if ($count['external']['nofollow'] == 0 && $count['external']['dofollow'] == 0) {
			$this->save_score_result(6, __('No outbound links appear in this page, consider adding some as appropriate.'), 'links');
		} else {
			$found = false;
			...
			if ($found) {
				$this->save_score_result(2, __('You\'re linking to another page with the keyword you want this page to rank for, consider changing that if you truly want this page to rank.'), 'links_focus_keyword');
			}
	
			if ($count['external']['nofollow'] == 0 && $count['external']['dofollow'] > 0) {
				$this->save_score_result(9, __f('This page has %s outbound link(s).', array($count['external']['dofollow'])), 'links_number');
			} elseif ($count['external']['nofollow'] > 0 && $count['external']['dofollow'] == 0) {
				$this->save_score_result(7, __f('This page has %s outbound link(s), all nofollowed.', array($count['external']['nofollow'])), 'links_number');
			} else {
				$this->save_score_result(8, __f('This page has %s nofollowed link(s) and %s normal outbound link(s).', array($count['external']['nofollow'], $count['external']['dofollow'])), 'links_number');
			}
		}
	}
	...
	private function score_headings($headings) {	
		...
		if ($headingCount == 0) {
			$this->save_score_result(7, __('No subheading tags (like an H2) appear in the copy.'), 'headings');
		} else {
			$found = 0;
			...
			if ($found) {
				$this->save_score_result(9, __f('Keyword / keyphrase appears in %s (out of %s) subheadings in the copy. While not a major ranking factor, this is beneficial.', array($found, $headingCount)), 'headings');
			} else {
				$this->save_score_result(3, __('You have not used your keyword / keyphrase in any subheading (such as an H2) in your copy.'), 'headings');
			}
		}
	}
	...
	private function score_images_alt_text($imgs) {
		if ($imgs['count'] == 0) {
			$this->save_score_result(3, __('No images appear in this page, consider adding some as appropriate.'), 'images_alt');
		} elseif (count($imgs['alts']) == 0 && $imgs['count'] != 0) {
			$this->save_score_result(5, __('The images on this page are missing alt tags.'), 'images_alt');
		} else {
			$found = false;
			...
			if ($found) {
				$this->save_score_result(9, __('The images on this page contain alt tags with the target keyword / phrase.'), 'images_alt');
			} else {
				$this->save_score_result(5, __('The images on this page do not have alt tags containing your keyword / phrase.'), 'images_alt');
			}
		}
	}
