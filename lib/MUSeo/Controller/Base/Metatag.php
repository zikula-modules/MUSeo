...
/**
 * Metatag controller base class.
 */
class MUSeo_Controller_Base_Metatag extends Zikula_AbstractController
{
    /**
     * Post initialise.
     *
    ...
    public function main()
    {
        ...
        $objectType = 'metatag';
        $utilArgs = array('controller' => 'metatag', 'action' => 'main');
        ...
        if ($legacyControllerType == 'admin') {
            ...
            $redirectUrl = ModUtil::url($this->name, 'metatag', 'view', array('lct' => $legacyControllerType));
        ...
        // set caching id
        $this->view->setCacheId('metatag_main');
        
        // return main template
        return $this->view->fetch('metatag/main.tpl');
    }
    ...
    public function view()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'metatag';
        $utilArgs = array('controller' => 'metatag', 'action' => 'view');
        ...
        $currentUrlObject = new Zikula_ModUrl($this->name, 'metatag', 'view', ZLanguage::getLanguageCode(), $currentUrlArgs);
    ...
    public function display()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'metatag';
        $utilArgs = array('controller' => 'metatag', 'action' => 'display');
        $permLevel = $legacyControllerType == 'admin' ? ACCESS_ADMIN : ACCESS_READ;
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel), LogUtil::getErrorMsgPermission());
        $entityClass = $this->name . '_Entity_' . ucfirst($objectType);
        $repository = $this->entityManager->getRepository($entityClass);
        ...
        $currentUrlObject = new Zikula_ModUrl($this->name, 'metatag', 'display', ZLanguage::getLanguageCode(), $currentUrlArgs);
    ...
    public function edit()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'metatag';
        $utilArgs = array('controller' => 'metatag', 'action' => 'edit');
        ...
        $handlerClass = $this->name . '_Form_Handler_Metatag_Edit';
    ...
    public function delete()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'metatag';
        $utilArgs = array('controller' => 'metatag', 'action' => 'delete');
        ...
        if (!$deleteAllowed) {
            return LogUtil::registerError($this->__('Error! It is not allowed to delete this metatag.'));
        }
        ...
        if ($confirmation && $deleteAllowed) {
            ...
            if (!$validators->hasErrors()) {
                ...
                // Let any hooks know that we have created, updated or deleted the metatag
                $hookType = 'process_delete';
                $hook = new Zikula_ProcessHook($hookAreaPrefix . '.' . $hookType, $entity->createCompositeIdentifier());
                $this->notifyHooks($hook);
        
                // The metatag was deleted, so we clear all cached pages this item.
                $cacheArgs = array('ot' => $objectType, 'item' => $entity);
                ModUtil::apiFunc($this->name, 'cache', 'clearItemCache', $cacheArgs);
        
                if ($legacyControllerType == 'admin') {
                    // redirect to the list of metatags
                    $redirectUrl = ModUtil::url($this->name, 'metatag', 'view', array('lct' => $legacyControllerType));
                } else {
                    // redirect to the list of metatags
                    $redirectUrl = ModUtil::url($this->name, 'metatag', 'view', array('lct' => $legacyControllerType));
    ...
    public function handleSelectedEntries()
    {
        ...
        $redirectUrl = ModUtil::url($this->name, 'admin', 'main', array('ot' => 'metatag'));
        ...
        foreach ($items as $itemid) {
            ...
            if ($action != 'delete') {
                ...
                $url = new Zikula_ModUrl($this->name, 'metatag', 'display', ZLanguage::getLanguageCode(), $urlArgs);
            }
    ...
    public function handleInlineRedirect()
    {
        ...
        $this->view->display('metatag/inlineRedirectHandler.tpl');
