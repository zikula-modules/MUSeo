 ...
 *
 * This is the base repository class for metatag entities.
 */
class MUSeo_Entity_Repository_Base_Metatag extends EntityRepository
{
    /**
     * @var string The default sorting field/expression.
     */
    protected $defaultSortingField = 'titleOfEntity';
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('metatag', 'workflowState');
                $templateParameters['robotsIndexItems'] = $listHelper->getEntries('metatag', 'robotsIndex');
                $templateParameters['robotsFollowItems'] = $listHelper->getEntries('metatag', 'robotsFollow');
                $templateParameters['robotsAdvancedItems'] = $listHelper->getEntries('metatag', 'robotsAdvanced');
            }
    
            // initialise Imagine preset instances
            $imageHelper = new MUSeo_Util_Image($serviceManager);
    
            $objectType = 'metatag';
            $templateParameters[$objectType . 'ThumbPresetFacebookImage'] = $imageHelper->getPreset($objectType, 'facebookImage', $context, $args);
            $templateParameters[$objectType . 'ThumbPresetTwitterImage'] = $imageHelper->getPreset($objectType, 'twitterImage', $context, $args);
            $templateParameters[$objectType . 'ThumbPresetGooglePlusImage'] = $imageHelper->getPreset($objectType, 'googlePlusImage', $context, $args);
            if (in_array($args['action'], array('display', 'view'))) {
                // use separate preset for images in related items
                $templateParameters['relationThumbPreset'] = $imageHelper->getCustomPreset('', '', 'MUSeo_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters['robotsIndex'] = isset($this->controllerArguments['robotsIndex']) ? $this->controllerArguments['robotsIndex'] : FormUtil::getPassedValue('robotsIndex', '', 'GET');
        $parameters['robotsFollow'] = isset($this->controllerArguments['robotsFollow']) ? $this->controllerArguments['robotsFollow'] : FormUtil::getPassedValue('robotsFollow', '', 'GET');
        $parameters['robotsAdvanced'] = isset($this->controllerArguments['robotsAdvanced']) ? $this->controllerArguments['robotsAdvanced'] : FormUtil::getPassedValue('robotsAdvanced', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MUSeo_Entity_Metatag', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUSeo_Entity_Metatag', 'tbl')
     ...
     *
     * @return array|MUSeo_Entity_Metatag retrieved data array or MUSeo_Entity_Metatag instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved MUSeo_Entity_Metatag instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                SessionUtil::setVar('MUSeoMetatagsCurrentPage', $page);
            } else {
                // restore current page from session
                $page = SessionUtil::getVar('MUSeoMetatagsCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('MUSeo', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'metatag', 'catids' => $v));
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if ($currentLegacyControllerType == 'admin' && $currentModule == 'MUSeo') {
            return $qb;
        }
    
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved metatags only
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= 'tbl.titleOfEntity LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.focusKeyword LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.robotsIndex = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.robotsFollow = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.robotsAdvanced = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.canonicalUrl = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.redirectUrl = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.facebookTitle LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.facebookDescription LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.facebookImage = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.twitterTitle LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.twitterDescription LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.twitterImage = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.googlePlusTitle LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.objectOfModule LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.nameOfIdentifier LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.stringOfObject LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.extraInfos LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.titleOfEntity LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numMetatags';
        if ($useJoins === true) {
     ...
     * @param int    $excludeId  Id of metatags to exclude (optional).
     *
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        ...
        $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('MUSeo', 'showOnlyOwnEntries', 0), 'GETPOST');
