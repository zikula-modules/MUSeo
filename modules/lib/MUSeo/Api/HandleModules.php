...
class MUSeo_Api_HandleModules extends MUSeo_Api_Base_HandleModules
{
	...
	public function setModuleMetaTags($modname, $modfunc = 'main')
	{
		...
		if (count($moduleinfo[0]['objectOfModules']) > 1) {
			
		}

		if ($moduleinfo[0]['controllerForSingleObject'] == $modfunc) {
			...
			$objectId = $request->getGet()->filter($moduleinfo[0]['nameOfIdentifier'], '', FILTER_SANITIZE_STRING);
		    if ($objectId != '') {
		    	$where .= ' AND ';
